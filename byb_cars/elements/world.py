...
class GameWorld:
    def __init__(self, baseline_speed=3.0, game_height=600):
        ...
        self.start_line_surface = self.create_checkerboard(self.road_width, self.start_line_height)
        self.finish_line_surface = self.create_checkerboard(self.road_width, self.finish_line_height, size=15)  # Smaller squares
    ...
    def create_road(self):
        ...
        while y < surface.get_height():
            pygame.draw.rect(surface, defaults.LINE_WHITE, 
                           (center_x - line_width//2, y, line_width, dash_length))
            y += dash_length + gap_length
        
        # Draw edge lines
        pygame.draw.rect(surface, defaults.LINE_WHITE, (5, 0, 3, surface.get_height()))
        pygame.draw.rect(surface, defaults.LINE_WHITE, (self.road_width - 8, 0, 3, surface.get_height()))
    ...
    def draw(self, surface, car_screen_y):
        ...
        pygame.draw.rect(surface, defaults.GRASS_GREEN, (0, 0, self.road_left, game_area_height))
        pygame.draw.rect(surface, defaults.GRASS_GREEN, (self.road_right, 0, defaults.WIDTH - self.road_right, game_area_height))
        ...
        if -self.start_line_height < start_y < game_area_height:
            ...
            surface.blit(text_bg, (self.road_left + self.road_width/2 - text.get_width()/2 - 5, start_y - 45))
            surface.blit(text, (self.road_left + self.road_width/2 - text.get_width()/2, start_y - 40))
        ...
        if -self.finish_line_height < finish_y < game_area_height:
            ...
            text_bg.fill((0, 0, 0))
            text_bg.set_alpha(180)  # Semi-transparent
            surface.blit(text_bg, (self.road_left + self.road_width/2 - text.get_width()/2 - 5, finish_y - 45))
            surface.blit(text, (self.road_left + self.road_width/2 - text.get_width()/2, finish_y - 40))
    ...
    def draw_timer(self, surface):
        ...
        surface.blit(time_surface, (defaults.WIDTH // 2 - time_surface.get_width() // 2, 20))
        ...
        if self.best_time is not None:
            ...
            surface.blit(best_surface, (defaults.WIDTH - best_surface.get_width() - 20, 20))
        ...
        pygame.draw.circle(surface, 
                         ...
                         (defaults.START_LINE_COLOR if self.race_started else (200, 200, 200)), 
                         (defaults.WIDTH // 2 - 50, indicator_y), 
                         indicator_radius)
                         
        status_font = pygame.font.SysFont(None, 24)
        start_text = status_font.render("Start", True, (0, 0, 0))
        surface.blit(start_text, (defaults.WIDTH // 2 - 50 + indicator_radius + padding, indicator_y - 10))
        ...
        surface.blit(finish_text, (defaults.WIDTH // 2 + 50 + indicator_radius + padding, indicator_y - 10))
